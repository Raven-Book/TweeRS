...
本指南将带你从零创建一个 Twine 故事，并用 TweeRS 编译成可运行的 HTML 文件。
...
<Steps>
  ...
  构建完成后，用浏览器打开 `game.html` 即可游玩你的 Twine 故事。
</Steps>
...
随着故事变长，你可以将片段拆分到多个 `.twee` 文件中。TweeRS 会自动合并目录下所有 `.twee` 和 `.tw` 文件：

```
my-story/
├── src/
│   ├── main.twee        # StoryTitle、StoryData 和 Start
│   ├── chapter1.twee    # 第一章的片段
│   └── chapter2.twee    # 第二章的片段
└── game.html            # 构建输出
```

:::tip
只需在一个文件中定义 `StoryTitle` 和 `StoryData`，其余文件只写片段即可。
:::

## 下一步

- [VSCode 插件](/guide/vscode) — 在编辑器内实时预览、片段导航与构建
- [扩展功能: 数据表](/guide/extensions/excel) — 从 Excel 导入数据到游戏中
- [扩展功能: 脚本注入](/guide/extensions/scripts) — 自定义构建流程
