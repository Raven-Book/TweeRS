...
Tweers Dev Tool 是 TweeRS 的 Visual Studio Code 插件，提供实时预览、片段导航、构建输出等功能，让你在编辑器内完成故事开发的完整流程。

:::tip 前置条件
请确保已完成 [安装](/guide/installation)，包括 VSCode 插件。
:::
...
**从指定片段运行**
- 选择任意片段（特殊片段除外），点击 `Run from here`
- 将该片段设为故事起点，重新构建预览
- 当前起点片段会显示黄色星标图标

<FileLink src="imgs/vscode/插件图-1.png" />
<FileLink src="imgs/vscode/插件图-2.png"/>

:::tip
`StoryData` 和 `StoryTitle` 是特殊片段，不支持「从此处运行」。
:::
...
| `tweers-dev-tool.passagesGroupBy` | `"none"` | 片段分组方式：`"none"` 或 `"file"` |
...
| `Tweers: Search Passages` | 搜索片段 |
| `Tweers: Clear Search` | 清除搜索 |
| `Tweers: Toggle Passages Grouping` | 切换片段分组方式 |

## 故事格式

插件从 `StoryData` 片段中读取 `format` 和 `format-version` 字段，自动在 `.storyformats/` 下创建对应目录。你需要将故事格式的 `format.js` 文件放入该目录：
