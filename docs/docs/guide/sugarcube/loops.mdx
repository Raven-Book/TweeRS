...
在循环里用 `<<link>>` 或 `<<button>>` 生成多个链接/按钮时，会遇到一个问题：在**点击时**才读取变量的值，而那时循环早已结束，变量停留在最后一次的值。

先看问题——点击任意一个链接，显示的都是最后一个水果：
...
`<<capture>>` 可以把变量的当前值"锁住"，让每个链接记住自己创建时的值：
...
规则很简单：在循环里用 `<<link>>` 或 `<<button>>` 时，用 `<<capture>>` 包裹并列出用到的临时变量。
