...
use super::nodes::basic::{
    DataAggregatorNode, FileChangeDetectorNode, FileCollectorNode, FileParserNode,
    FileWriterNode, HtmlGeneratorNode,
};
...
pub fn register_nodes(registry: &mut NodeRegistry) {
    ...
    registry.register("file_change_detector", || {
        Box::new(FileChangeDetectorNode)
    });
